...
{
	TRANS( 0,0,0 ) = m_exkid*m_K[0];		// for S0,0,0 = k0(k*d)*exp(-k*d)/d
	m_exkid *= m_ir;

	if ( m_bGrad )
		dTRANS( 0,0,0 ) = -m_exkid*m_K[1];

	m_exkid *= m_scalei;  //was CMCoeff::RS, exp term now also contains the (lam/d)^l term

	for ( int l = 1; l < 2*m_p-1; l++ )
	{
		TRANS( l,0,0 ) = m_exkid*m_K[l];	// for S l,0,0 = (lam/d)^l*kl(k*d)*exp(-k*d)/d
		m_exkid *= m_ir;

		if ( m_bGrad )										// EQ 1.10 in Lotan 2006
...
CTransCoeff::computeCoeff_( vector<vector<vector<REAL> > > & U )
{
	...
	for ( int n = 1; n < m_p-1; n++ )				// EQ 1.6, Lotan 2006
	...
	for ( int m = 1; m < m_p; m++ )
	{
		...
		for ( int n = m+1; n < m_p-1; n++ )				// EQ 1.8, Lotan 2006
			for ( int l = n+1; l < 2*m_p-2-n; l++ )
			{
				U[l][n+1][m] = -( kpio*BETA(l-1,m )*U[l-1][n][m] +
...
******************************************************************/
REAL
CTransCoeff::computeError( const CMulExpan & tM1, 
			const CMulExpan & tM2, int p )
{
	assert( p <= m_p );
...
CTransCoeff::exportMat( double ** T )
{
	...
	for ( int m = -n; m <= n; m++ ) 
	{
		...
		for ( int l = abs(m ); l < m_p; l++)
		{
			...
			T[j][k] = ( EVEN(n+l ) ? TRANS(n,l,abs(m)) 
							: -TRANS( n,l,abs(m )));
		}
	}
}	// end exportMat
...
CTransCoeff::checkT( int l, int n, int m )
{
	...
	if( !bN || !bL || !bM  ) 
	{
		cout <<"Error with transcoeff T index access: "
						<<bN<<" "<<bL<<" "<<bM<<endl;
	...
	if( bsL ) 
	{
		cout <<"Transcoeff T too small: l= "<<l<<" "<<sL<<endl; 
		return false;
	}
	int sN = m_T[l].size(  );
	bool bsN = ( n >= sN  );
	if( bsL )
	{
		cout <<"Transcoeff T too small: n="<<n<<" "<<sN<<endl; 
		return false;
	}
	int sM = m_T[l][n].size(  );
	bool bsM= ( m >= sM  );
	if( bsM )
	{
		cout <<"Transcoeff T too small: m="<<m<<" "<<sM<<endl; 
		return false;
	}
