...
class CSpPnt
{
  // A 3-D point or vector expressed in spherical coordinates
 public:
  CSpPnt() 
    { zero(); }
...
class CPnt
{
  // A 3-D point or vector in cartesian coordinates
 public:
  CPnt() 
    { zero(); }
  ...
  // Arithmetic operators applied to this vector
...
class CQuat
{
  // A quaternion used to describe a rotation
 public:
  ...
  */
  ...
  CQuat(const double *rotm)
    {
	m_real  = sqrt(1.0 + rotm[0] + rotm[4] + rotm[8]) / 2.0;
	double w4 = (4.0 * m_real);
	double x = (rotm[7] - rotm[5]) / w4 ;
	double y = (rotm[2] - rotm[6]) / w4 ;
	double z = (rotm[3] - rotm[1]) / w4 ;
	m_imag = CPnt(x,y,z);
	normalize();
    }
  ...
  // Access the different components of the quaternion
  REAL x() const
  { return m_imag.x(); }
  REAL y() const
  { return m_imag.y(); }
