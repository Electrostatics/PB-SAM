		  ...
		  for(int h=0; h<sphereListJ.size(); h++)
		    {
		      ...
		      //		      double allowedOverlap = (MAXSPHERE_OVERLAP[ki] < MAXSPHERE_OVERLAP[kj] ? MAXSPHERE_OVERLAP[ki] : MAXSPHERE_OVERLAP[kj] );
...
CMolecule::OutsideAllBoundaries(const vector<CMolecule*> & mols, CPnt P)
{
  ...
  // for(int i=0; i<mols.size() && bOut ; i++) bOut = !( mols[i]->IsInsideSurface(CPnt P) );
...
//      if( c2cdist < mols[i]->getMaxR() + mols[j]->getMaxR()  ||  !(bAggI || bAggJ)  )
//	{
...
//   cout<<"mutual polarzation done"<<endl; 
...
//     cout<<i<<"th molecule force calculated"<<endl;
  ...
  */
  ...
  for(int i=0; i<nmol; i++)
      ...
      if(mols[i]->getDockedNeigh().size() > 0)
	{
	  ...
	  //cout <<n<<") mol "<<i<<" has "<<mols[i]->getDockedNeigh().size()<<" docked neighbors"<<endl;
	}
    }
