 ...
 * # Date: June 2013
 * #
 * # Description: Molecule functions?
 * #
...
#define MAX_POLAR_DEV ( 1e-2 )//(2e-4)//(1e-4)
...
/*#########################################################*/
/*#########################################################*/
// create a molecule from scratch with 
// just sphere centers and radii
			...
			m_molcells( molcell ) 
{
	...
	int ki, tid;
	#pragma omp parallel shared( rad2 ) private(ki, tid) // !! ALBAUGH
	{   
		...
		for( ki = 0; ki < getNKS( ); ki++ )
		{
			...
			//getXFormSpherePoints( cens_r, radii, rad2, SPdum, 
			//			NPdum, neigh, ki, SPx, nSPx, NUM_POINTS_X );
				...
				m_molcells( molcell ) 
{
	...
	int ki, tid;
	#pragma omp parallel private( ki, tid ) // !! ALBAUGH
	{   
		#pragma omp for // !! ALBAUGH
		for( ki = 0; ki < getNKS( ); ki++ )
		{
...
/*#########################################################*/
/*#########################################################*/
// this function will generate lists for 
// intra-molecular polarization
// Add a comment to this line
// if the distance between sphere ki and kj is 
// less than intraRcutoff,
// they will be put into intraPolLists_near, 
// where the local expansion
// will be performed numerically ( see Eq. 27a, 28b in 2010 JCTC paper ).
// Otherwise they will be  put into 
// intraPolLists_far, where the local
		   ...
		   vector<double> &totalF, vector<double> &totalH )
{
	...
	for( int ki=0; ki<ncen; ki++ ) 
	{
		...
		//      			qSolvedH[ki][0], totalF[ki], totalH[ki] );
	}
 ...
 const vector<CPnt> &cpos, vector<int> &clabel )
{
	...
	for( int i=0; i<cpos.size( ); i++ )
	{
		...
		//{  cout <<"Error! charge "<<i<<" at "<<cpos[i]<<
		//		" not assigned to any center!"<<endl; exit( 1 ); }
	}
	return;
}	// end assignCharges
...
CMolecule::polarize_self( bool bPot, int farFieldFreq )
{
	...
	{      
		...
		#pragma omp parallel for reduction( +:dev ) shared(NKis) // !! ALBAUGH
		for( ki=0; ki < NKis; ki++ ) 
		{
			REAL dev_ki = 0; 
