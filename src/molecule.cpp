...
// tolerance for iterative solution of F and H ( S. Liu )
...
const int NMIN = 1000;												//!<
const int NMAX = 72000;												//!<
const int NP_PER_OSCILLATION = 5;							//!< Number of poles per oscillation

// Static Variables
double CMolecule::m_sdiel;
double CMolecule::KAPPA;
double CMolecule::SPHERETOL = 0.0;						//!< Sphere tolerance for extracting charges

double CMolecule::m_interRcutoff = DBL_MAX;
double CMolecule::m_interactRcutoff = DBL_MAX;
double CMolecule::INTRA_RCUTOFF = 30;
double CMolecule::MAXDEV = 0.0;
int CMolecule::NUM_POINTS_I;										//!< number of surface points for calculation of Imat
int CMolecule::NUM_POINTS_X;										//!< number of surface points for calculation of Imat
bool CMolecule::m_bGrad = false;								//!< A boolean for whether or not to compute the gradient
int CMolecule::N_MOL = 0;												//!< Number of molecules in the system
...
CMolecule::generateIntraMolPolList( vector<CMolecule*> & mols  )
{
	...
	for( int j=0; j < nmol; j++ )
	{
		for( int kj=0; kj<mols[j]->getNKS( ); kj++ )
		{
			...
			// add spheres that have no inter-pol neighbors,
			// but have inter-interact neighbors
...
CMolecule::writeMolsXYZR( const char * fname, const vector<CMolecule*> & mols )
{
	...
	for( int i = 0; i < mols.size( ); i++ ) sphere_tot += mols[i]->getNKS();
	fout << sphere_tot << endl;
	fout << "ATOMS" << endl;
	...
	for( int i = 0; i < mols.size( ); i++ )
	{
		...
		for( int ki = 0; ki < mols[i]->getNKS( ); ki++ )
		{
			...
			sprintf( buf, "%8.3f%8.3f%8.3f%6.2f", pos.x( ), pos.y(),
					pos.z(  ), mols[i]->getKS(ki).getRad());
			fout << buf << endl;
		}// endki
	}// endi
...
#pragma omp for
		for( ki = 0; ki < getNKS( ); ki++ )
		{
			...
			cout <<"SPE: exposed surface points "<<SPE.size(  )<<" SPB: buried surf pts "<<SPB.size()<<endl;
			
			// extract charge and positions
			extractCharges( ki, cens_r[ki], m_cpos, m_chg, clabel,
...
: m_rot( false ), m_p(N_POLES), m_idiel(idiel),
m_bKappa(false), m_bAggregateM(false),
m_molcells( molcell )
{
...
CMolecule::CMolecule( int moltype, CPnt rcen,
					 ...
					 const vector<CMolCell> &molcell )
: m_rot( false ), m_p(N_POLES), m_idiel(idiel), m_bKappa(false),
m_bAggregateM(false), m_moltype(moltype),
m_molcells( molcell )
{
	assert( iMats.size( ) == cens.size());
	m_rot.reset( CQuat( ), m_p);
	...
	// Calculating the total charge of a molecule
	m_chg_sum = 0.0;
...
{
	CMolecule::generateMolSPX( scens, srad, SPxes, nSPx, neighs  );
	CMolecule::generateMolExposedChargesFH( Fself, Hself, scens, srad, SPxes, nSPx,
										   qSolvedF, qSolvedH, totalF, totalH );
	CMolecule::generateMolCells( scens, srad, rcen, molcells  );
	
	CMolecule::generateMolTypeIntraPolLists( scens, srad, intraRcutoff,
											intraPolLists_near, intraPolLists_far );
	
	CMolecule::computeMolTypeValues( rcen, scens, srad, chg, cpos, idiel,
									intraRcutoff, SPxes, nSPx, neighs,
									Fself, Hself, qSolvedF, qSolvedH, totalF, totalH,
									intraPolLists_near, intraPolLists_far,
									LFs_intraSelf, LHs_intraSelf,
									LFs_intraSelf_far, LHs_intraSelf_far );
	return;
}	// end generateAll
 ...
 ******************************************************************/
void
CMolecule::generateMolTypeIntraPolLists( const vector<CPnt> &cens,
...
CMolecule::computeMolTypeValues( CPnt rcen, const vector<CPnt> &cens,
								...
								const vector<CPnt> &cpos, double idiel,	REAL intraRcutoff,
								const vector<vector<CPnt> > &SPxes, const vector<int> &nSPx,
								const vector<vector<int> >&neighs,
								const vector<CMulExpan*> &Fself, const vector<CMulExpan*> &Hself,
								const vector<vector<REAL> > &qSolvedFself,
						  ...
						  vector<vector<int>> &neighs )
{
	...
	for( int ki = 0; ki < ncen; ki++ )
		rad2[ki] = srad[ki]*srad[ki];				// for each sphere in molecule, rad2 = rad^2
									   ...
									   vector<double> &totalF, vector<double> &totalH )
{
	...
	for( int ki=0; ki<ncen; ki++ )
	{
		...
		for( int n=0; n<N_POLES; n++ )
			constH[n] = CSolExpCenter::CONST2[n] / bessel_I[n];
...
/******************************************************************/
/**
 * Function to generate sphere points
 ******************************************************************/
void
...
/******************************************************************/
/**
 * itay's method: generate surface points for sphere i ( wrt to center i )
...
CMolecule::getXFormSpherePoints( const vector<CPnt> &cens, const vector<double> &radii,
								...
								const vector<CPnt> &NP,  const vector<int> &neigh, int ki,
								vector<CPnt> &SPx, int &nSPx, int Nin )
{
	...
	// Determine the number of points to generate, N, proportional to the size of the sphere
						 ...
						 const vector<CPnt> &cpos, vector<int> &clabel )
{
	...
	for( int i=0; i<cpos.size( ); i++ )
	{
		...
		if( clabel[i] == -1  )
		{  cout <<"Error! charge "<<i<<" at "<<cpos[i]<<" not assigned to any center!"<<endl; }
	}
	return;
}	// end assignCharges
				...
				for( int ki=0; ki<NKi; ki++ )
				{
					...
					if(   sepdist < m_interactRcutoff  )
					{
						...
						if( !mols[i]->getbInterXForm( ))
							mols[i]->setInterXForm(true);
						if( !mols[j]->getbInterXForm( ))
							mols[j]->setInterXForm(true);
					}//if-<minsep
				}//ki
			}//i
		}//kj
...
#if __DEBUGDIE__
	bool bH   = m_k[ki]->getH(  ).isBlownup() ;
	if(bH) cout <<"died in selfrecompute :H"<<endl;
	bool bLHS = m_k[ki]->getLHS(  ).isBlownup();
	if(bLHS) cout <<"died in selfrecompute :LHS"<<endl;
	bool bLFS = m_k[ki]->getLFS(  ).isBlownup();
	if(bLFS) cout <<"died in selfrecompute :LFS"<<endl;
...
CMolecule::reexpandLSFromSelfVal_LowMemory( const vector<CMolecule*> & mols, int i, int ki )
{
	...
	for( int k=0; k<plist.size( ); k++ )
	{
		...
		if( tL.isBlownup( ))
		{
			...
			cout <<"centers:"<< cen_ki<<" "<<cen_kj<<"P= "<<P
			<<" sepdist:"<<sepdist<<" pj2i="<<pj2i<<endl;
			cout <<"tL blown in reexpandLSFromSelfVal_LowMemory: "
			<<j<<" "<<kj<<"->"<<i<<" "<<ki<<endl;
			cout <<"tL\n"<<tL<<endl;
								...
								int j, vector<CGradExpan> &tG )
{
	...
	//==========================================================
	// then consider spheres in mol_j; i.e. i == j =>
	...
	for ( int hh = 0; hh <j_intrasize; hh++, k++ )
	{
		...
		for( int h=0; h<irlist.size( ); h++ )
		{
			...
			CPnt cen_km = mols[m]->getRCen(  ) + pKm->getCenRot();
			CPnt P = cen_ki - cen_km;
			int pm2i = CXFormBase::computeOrder( P.norm( ),
												pKm->getTQH(), pKm->getRad());
			CXFormA XA( *pKm, *pKi, true );
			XA.reset( P,pm2i );
									 ...
									 int j, vector<CGradExpan> &tG )
{
	...
	// then consider spheres in mol_j; i.e. i == j => del_( j,kj )T(i,ki <- m,km)
	// = del_(i,ki)T(i,ki <- m,km)
									   ...
									   map<CFullSphereID, int, classCompareCFullSphereID> &gHmap )
{
	...
	vector<int> plist;
	if( i==j )
		plist = mols[j]->getIntraMolPolList();
	else
	{
		vector<CFullSphereID> interMolPolList = mols[j]->getInterMolPolList(  );
...
#pragma omp for reduction( +:dev ) reduction(+:mtotal) schedule(dynamic) // !! ALBAUGH
	...
	{
		if (maxdevs[i] > MAXDEV)
			MAXDEV = maxdevs[i]; // record max deviation
	}
	...
	for ( int j = 0; j < i_max; j++ )
	{
		...
		else
		{
			...
			for ( k=0; k < j_intersize; k++ ) // inter
			{
				int i   = interMolPolList[k].mid(  );
				int ki  = interMolPolList[k].kid(  );
				d[k] = mols[i]->recomputeGrad_LowMemory( mols, tG[k], i, ki,
														j, false, tGFs, tGHs, tGHrots, gHmap );
				dev += d[k];
			}
#endif
			for ( int h=0; h < j_intrasize; h++, k++ ) // intra
...
#else
				d[k] = mols[j]->recomputeGrad_LowMemory( mols, tG[k], j, kj, j,
	...
	for ( int i=0; i<mols.size( ); i++ )
	{
		for ( int ki=0; ki<mols[i]->getNKS( ); ki++ )
		{
			...
			if( !mols[i]->getKS(ki ).IsEmptyInterPolList() )
			{
				...
				if( LS[0] > max || isnan(LS[0] ) )
				{  cout <<"LS > max"<<endl; bLS = true;}
			}
			
			if(  (mols[i]->getInterMolPolList( )).size() > 0)
			{
				if( fabs(gLHN[0][0] ) > max || fabs(gLHN[1][0]) > max
				   || fabs(gLHN[2][0]) > max || isnan( gLHN[0][0] )
				   || isnan(gLHN[1][0]) || isnan(gLHN[2][0]) )
				{
...
/******************************************************************/
/**
 * Generate a set of N points uniformly
 ...
 ******************************************************************/
void
CMolecule::extractCharges( int ki, CPnt cenKi, const vector<CPnt> &cpos,
...
/******************************************************************/
/**
 * extract charge and positions, save all point charge coords WRT current
 CG sphere
 ******************************************************************/
void
CMolecule::extractCharges( int ki, CPnt cenKi, const vector<CPnt> &cpos,
						  const vector<double> &chg,
