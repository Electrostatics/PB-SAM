...
CRotCoeff::initConstants()
{
  for (int n = 0; n < 2*N_POLES; n++)
    for (int m = 0; m <= n; m++)
      ZETA(n,m) = sqrt((REAL)(n+m+1)*(n-m+1)/((2*n+1)*(2*n+3)));
...
CRotCoeff::reset(const CQuat & Q, int p)
{
  ...
  //cout<<"molecule rotation : Q "<<Q<<" theta " << theta <<" phi "<<phi<<" xi "<<xi<<" p "<<p<<endl;
  reset(theta, phi, xi, p);
}
...
CRotCoeff::initParams(REAL theta, REAL phi, REAL xi)
{
  ...
  //cout <<"rotcoeff initparams theta "<<theta<<" phi "<<phi<<"  xi "<<xi<<endl;
...
// !!! Note: used only for general rotation e.g. molecular rotation. 
...
CRotCoeff::rotateWithXi(const CExpan & Min, CExpan & Mout, int p1, int p2, 
		  bool bFor) const 
{
  //   cout<<"In rotate:"<<(Min.getRange().p2())<<" "<<p2<<" "<<m_p<<endl;
  ...
  if (isSingular())
    {
      ...
      if (m_cost == -1.0)
	{
	  //	  cout<<"called rotatexi : pi case"<<endl;
	...
	{
	  //	  cout<<"called rotatexi : 0 case"<<endl;
	  for (int n = p1-1; n < p2; n++)
	    {
	      Mout(n,0) = Min(n,0);		  
	...
	{
	  //	  cout<<"called rotatexi : tpose"<<endl;
	  for (int n = p1 - 1; n < p2; n++)
	    for (int s = 0; s <= n ; s++)
	      {
...
// !!! Note: used only for rotation as part of RSR transform, 
// Has inbuild assumptions about values of xi for singular cases.
// Apply the rotation operator to the MP coeffs
void
		  ...
		  bool bFor) const
{
  ...
  if (isSingular())
	  ...
	  for (int n = p1-1; n < p2; n++)
	    {
	      ...
	      for (int m = 1; m <= n; m++) 
		{
		  Complex outcplx( Min(n,2*m-1) , Min(n,2*m) );
		  Mout(n,2*m-1) = s*outcplx.real();
		  Mout(n,2*m)   = -s*outcplx.imag();
  ...
  else // Not Singular
    {
      if (bFor)
	{
	  //cout <<"rotate bforward non singular "<<endl;
	  for (int n = p1 - 1; n < p2; n++)
	    for (int m = 0; m <= n; m++)
	      {
		//	cout <<" --- "<<n<<" "<<m<<endl;
		...
		//cout <<"s=0: incplx "<<incplx<<" ROT "<< ROT(n,0,m)
		//  <<" outcplx " <<outcplx<<endl;
		for (int s = 1; s <= n; s++) 
		  {
		    incplx = Complex( Min(n,2*s-1) , Min(n,2*s) );
		    outcplx += (ROT(n,s,m)*incplx + ROT(n,-s,m)*conj(incplx)); 
		    // cout <<"outcplx s "<<s<<" "<<outcplx<<endl;
		    /*
		    if(isnan(incplx.real()) || isnan(incplx.imag() ) )  
		      {cout <<"rot-for in " <<n<<" "<<s<<" "<<Min(n,2*s-1) <<" "<< Min(n,2*s)<<endl;}
		    if(isnan(outcplx.real()) || isnan(outcplx.imag() )) 
		      {cout <<"rot-for out "<<n<<" "<<s<<" "<<ROT(n,s,m)<<" "<< ROT(n,-s,m)<<endl;}
		    */
		      }
      ...
      else // TRANSPOSED
	{
	  ...
	  for (int n = p1 - 1; n < p2; n++)
		...
		for (int m = 1; m <= n; m++) {
		  ...
		  /*
		  if(isnan(incplx.real())  || isnan(incplx.imag()))  
		    {cout <<"rot-tpose in " <<n<<" "<<s<<" "<<m<<" "<<Min(n,2*m-1) <<" "<< Min(n,2*m)<<endl;}
		  if(isnan(outcplx.real()) || isnan(outcplx.imag()) )  
		  {cout <<"rot-tpose out "<<n<<" "<<s<<" "<<m<<" "<<ROT(n,s,m)<<" "<< ROT(n,-s,m)<<endl;}
		  */
...
// !!! Only used as part of RSR transform. Singular cases has inbuild assumption
// about values of xi

void 
CRotCoeff::dRotateTSing(const CExpan & Min, CExpan & Mout, int p1, int p2) const
{
  assert(Min.getRange().p2() >= p2 && p2 <= N_POLES && p2 <= m_p);
  assert(p1 == 1 || Mout.getRange().p2() == p1 - 1);  
  assert(p1 != 0 && p1 <= p2);
...
void 
CRotCoeff::dRotatePSing(const CExpan & Min, CExpan & Mout, int p1, int p2) const
{
  assert(Min.getRange().p2() >= p2 && p2 <= N_POLES && p2 <= m_p);
  assert(p1 == 1 || Mout.getRange().p2() == p1 - 1); 
  assert(p1 != 0 && p1 <= p2);
...
CRotCoeff::computeQCoeff()
{
  ...
  // outputs Q_coeff
  /*
  cout << "****Q COEFFICIENTS****" << endl;
  for (int m = 0; m < N_POLES-1; m++)
    {
      cout << "\t---m = " << m << "---" << endl;
      for (int n = m; n < 2*N_POLES-1; n++)
	{
	  for (int s = 0; s <= 1; s++)
	    {
	      REAL r = fabs(m_Q[n][m][s])>1e-15 ? 
		m_Q[n][m][s]: 0;
	      cout << r << " | ";
	    }
	  cout << endl;
...
CRotCoeff::checkR(int n, int s, int m) const
{
  ...
  bool bsN = (n < sN );
  if(!bsN)
    {cout <<"R too small: n="<<n<<" "<<sN<<endl; exit(0); /*return false;*/}

  int sS = m_R[n].size();
  bool bsS=( s < sS);
  if(!bsS)
    {cout <<"R too small: s= "<<s<<" "<<sS<<endl; exit(0); /*return false;*/}

  int sM = m_R[n][s].size();
  bool bsM= (m < sM );
  if(!bsM)
    {cout <<"R too small: m="<<m<<" "<<sM<<endl; exit(0); /*return false;*/}
...
CRotCoeff::checkdR(int n, int s, int m) const
{
  ...
  bool bsN = (n < sN );
  if(!bsN)
    {cout <<"dR too small: n="<<n<<" "<<sN<<endl; exit(0); /*return false;*/}

  int sS = m_dR[n].size();
  bool bsS=( s < sS );
  if(!bsS)
    {cout <<"dR too small: s= "<<s<<" "<<sS<<endl; exit(0); /*return false;*/}

  int sM = m_dR[n][s].size();
  bool bsM= (m < sM );
  if(!bsM)
    {cout <<"dR too small: m="<<m<<" "<<sM<<endl; exit(0); /*return false;*/}
